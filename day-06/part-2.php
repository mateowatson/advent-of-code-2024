...
foreach($room as $r_idx => $row) {
    foreach($row as $c_idx => $col) {
        ...
        while($stop === false) {
            // var_dump('a', $c_idx, $r_idx, $col);

            $last_pos = $guard_pos;
            $guard_pos = get_new_guard_pos();

            $last_two_pos = json_encode([json_encode($last_pos), json_encode($guard_pos)]);

            $is_looping = str_contains(trim(json_encode($guard_path), '[]'), trim($last_two_pos, '[]'));
